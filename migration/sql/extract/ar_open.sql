-- Open AR, derived from the receivable lines themselves rather than from the
-- invoice header, so that AR ties to the GL by construction.
SELECT
    aml.id                                  AS source_id,
    am.name                                 AS number,
...
WHERE am.company_id = %(company_id)s
  ...
  AND aa.account_type = 'asset_receivable'     -- Odoo <= 14: internal_type = 'receivable'
